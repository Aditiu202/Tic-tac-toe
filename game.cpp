...
{
cout<<"You can insert in the following position:";
for(int i=0; i<SIDE; i++)
for (int j = 0; j < SIDE; j++)
if (board[i][j] == '*')
cout<<","<<((i*3+j) +1);
...
{
board[x][y] = HUMANMOVE;
cout<<"HUMAN has put a :  "<< HUMANMOVE <<" in cell => "<<  n+1 <<endl<<endl;
showBoard(board);
moveIndex ++;
whoseTurn = COMPUTER;
}
...
{
cout<<endl<<"Position is occupied, select any one place from the available places\n\n";
}
else if(n<0 || n>8)
{
